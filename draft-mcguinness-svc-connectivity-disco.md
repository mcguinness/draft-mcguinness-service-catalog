...
* Lowered the Service Category registry policy from Specification Required to Expert Review, since a category is a descriptive label rather than something that changes client behavior. The protocol-affecting registries (service type, connection profile, OAuth connection type) keep Specification Required.
* Tightened the trust model around `connected`: the `status` definition now states it is the provider's assertion that an account or linkage exists, not that a credential is in hand, the service is the intended one, or calling is safe, and that it does not waive the {{profile-oauth}} checks. Reframed Authority for Inclusion as two explicit levels, discoverability authorization (what the catalog asserts) versus runtime authorization (the service and authorization server's decision at call time).
* Follow-up review cleanups: noted that `client_registration` values are extended by specifications updating the `oauth` profile and that an unknown value is not used; removed the duplicated `client_id`/`static` rationale and stated when `client_id` is present or absent; aligned the `authorization_code` client-identity wording with the explicit modes; noted that `client_credentials` satisfies resource confirmation by local policy; and split a remaining dense sentence.
